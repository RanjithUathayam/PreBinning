...
@Component({
  selector: 'app-bin-consolidation',
  templateUrl: './bin-consolidation.page.html',
  styleUrls: ['./bin-consolidation.page.scss'],
})
export class BinconsolidationPage implements OnInit, AfterViewInit {
  binId: string = '';
  itemCode: string = '';
  ...
  addItem() {
    ...
    if (this.grndataArr && this.grndataArr.length > 0) {
      ...
      const currentTotalQuantity = this.grndataArr.reduce((total, item) => total + item.Quantity, 0);
    ...
    if (this.itemAlreadyScanned.includes(this.itemCode) && this.itemAlreadyScanned.length) {
    ...
    const existingItem = this.grndataArr.find(item => item.ItemCode === scannedItemCode);
    if (existingItem) { 
      existingItem.Quantity += scannedQuantity;
      existingItem.Count++; 
      this.grndataArr = this.grndataArr.filter(item => item.ItemCode !== scannedItemCode);
      this.grndataArr.unshift(existingItem);
    } else {
  ...
  handleSubmit() { 
    ...
    this.loadingService.presentLoading();
    this.apiservice.updaterefilledbin(obj).subscribe((response: any) => {
        this.loadingService.dismissLoading();
        if (response.status === 1) {
      ...
      (error) => {
        this.loadingService.dismissLoading();
        this.presentAlert('Error while Refilling data.');
      }
    );
